...
const ROUTINE_SELECT_WITH_SCHEDULE = `${ROUTINE_SELECT_LEGACY}, schedule_mode`;
const ROUTINE_SELECT_WITH_PROGRESSION = `${ROUTINE_SELECT_WITH_SCHEDULE}, default_progression_playbook_id, default_progression_playbook_config`;
...
export default async function EditRoutinePage({ params, searchParams }: PageProps) {
  ...
  const { data: routineWithProgression, error: routineWithProgressionError } = await supabase
    ...
    .eq("user_id", user.id)
    .single();
  const { data: routineWithSchedule, error: routineWithScheduleError } = routineWithProgressionError
    && (isMissingRoutineDefaultProgressionColumnError(routineWithProgressionError) || isMissingRoutineScheduleColumnError(routineWithProgressionError))
    ? await supabase
        .from("routines")
        .select(ROUTINE_SELECT_WITH_SCHEDULE)
        .eq("id", params.id)
        .eq("user_id", user.id)
        .single()
    : { data: null, error: null };
  const { data: legacyRoutine } = routineWithScheduleError && isMissingRoutineScheduleColumnError(routineWithScheduleError)
    ? await supabase
        .from("routines")
        ...
        .single()
    : { data: null };
  const routine = routineWithProgression ?? routineWithSchedule ?? legacyRoutine;
  ...
  return (
    <RoutineDetailsScreenShell backHref={returnHref} title={(routine as RoutineRow).name} align="center">
      <EditRoutineAutosaveForm
        ...
        scheduleMode={(routine as RoutineRow).schedule_mode ?? "weekday_anchored"}
