...
import { dispatchFeedbackInteraction } from "@/lib/discord/runtime/domains/feedback";
...
const DISCORD_MESSAGE_COMMAND_RELEASE_CHECK_TRIGGERS = [
...
function discordMessageRequestsFeedbackReactionSync(message: DiscordMessageCommand): boolean {
  const normalizedContent = normalizeDiscordMessageCommandContent(message.content);
  return DISCORD_MESSAGE_COMMAND_FEEDBACK_REACTION_SYNC_TRIGGERS.some((trigger) => normalizedContent.includes(trigger));
}
...
function discordMessageRequestsComputaReleaseCheck(message: DiscordMessageCommand): boolean {
  const normalizedContent = normalizeDiscordMessageCommandContent(message.content);
...
function parseDiscordComputaUpdateMessageCommand(message: DiscordMessageCommand): DiscordComputaUpdateCommand | null {
  if (typeof message.content !== "string") {
    return null;
  }

  const commandMatch = message.content.trim().match(/^computa\s+post\s+update(?:\s+([\s\S]+))?$/i);
  const rawArgument = commandMatch?.[1]?.trim() ?? "";
  if (!commandMatch || !rawArgument) {
    return null;
  }
  ...
  const pipeIndex = argument.indexOf("|");
  if (pipeIndex > 0) {
    const title = argument.slice(0, pipeIndex).trim();
    const description = argument.slice(pipeIndex + 1).trim();
  ...
  return {
    title: "Fawx Security update",
    description: argument.slice(0, 1200),
  };
}

function discordMessageRequestsComputaUpdate(message: DiscordMessageCommand): boolean {
  if (typeof message.content !== "string") {
    return false;
  }
...
function buildDiscordComputaFormattedUpdatePayload(command: DiscordComputaUpdateCommand): Record<string, unknown> {
  return {
    content: "",
    allowed_mentions: {
      parse: [],
...
function resolveDiscordForumTagIdsByNames(channel: { available_tags?: unknown }, tagNames: string[]): string[] {
  ...
  return tags
    .filter((tag): tag is { id: string; name: string } => {
      ...
      return typeof candidate.id === "string"
        ...
        && wantedNames.has(candidate.name.toLowerCase());
    })
    .map((tag) => tag.id);
}
...
async function syncDiscordFeedbackResolvedReactions() {
  ...
  const resolvedTagIds = new Set(resolveDiscordForumTagIdsByNames(forumResult.channel, DISCORD_FEEDBACK_RESOLVED_TAG_NAMES));
...
function discordMessageHasComputaReleaseCheck(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  ...
  return embeds.some((embed) => {
    ...
    const footer = "footer" in embed ? (embed as { footer?: unknown }).footer : undefined;
    if (footer && typeof footer === "object" && "text" in footer) {
      return (footer as { text?: unknown }).text === DISCORD_COMPUTA_RELEASE_CHECK_MARKER;
    }

    return "title" in embed && (embed as { title?: unknown }).title === "Computa Release Check";
  });
}

async function buildDiscordReleaseLedgerCheckPayload(): Promise<
  | { ok: true; body: Record<string, unknown>; issueCount: number }
  | { ok: false; code: string; status?: number; message?: string | null }
> {
  const forumChannelId = DISCORD_BUG_REPORT_FORUM_CHANNEL_ID();
  if (!forumChannelId) {
  ...
  for (const thread of threadsResult.threads) {
    ...
    const appliedTagIds = Array.isArray(thread.applied_tags) ? thread.applied_tags : [];
    const isResolved = appliedTagIds.some((tagId) => resolvedTagIds.has(tagId));
    const hasCustomSuccess = discordMessageHasCustomSuccessReaction(starterResult.message as DiscordMessageCommand);
    const hasLegacySuccess = discordMessageHasLegacySuccessReaction(starterResult.message as DiscordMessageCommand);
      ...
      embeds: [
        {
          ...
          footer: {
            text: DISCORD_COMPUTA_RELEASE_CHECK_MARKER,
          },
        },
      ],
    },
...
async function processDiscordReleaseLedgerCheckMessageCommand(args: {
  channelId: string;
  message: DiscordMessageCommand;
}) {
  const authorization = await authorizeDiscordCommanderMessageCommand(args);
  ...
  await markDiscordMessageCommandProcessed({
    channelId: args.channelId,
    messageId: authorization.messageId,
    emoji: payloadResult.issueCount === 0 ? DISCORD_MESSAGE_COMMAND_SUCCESS_REACTION : DISCORD_MESSAGE_COMMAND_WARNING_REACTION,
  });
  return { ok: true as const, action: "checked", issueCount: payloadResult.issueCount };
}
...
async function processDiscordFeedbackReactionSyncMessageCommand(args: {
  channelId: string;
  message: DiscordMessageCommand;
}) {
  const authorization = await authorizeDiscordCommanderMessageCommand(args);
...
async function processDiscordComputaUpdateMessageCommand(args: {
  channelId: string;
  message: DiscordMessageCommand;
}) {
  const messageId = typeof args.message.id === "string" ? args.message.id : null;
  ...
  const command = parseDiscordComputaUpdateMessageCommand(args.message);
  if (!command) {
    await markDiscordMessageCommandProcessed({
      channelId: args.channelId,
      messageId,
    ...
    return { ok: false as const, code: "DISCORD_COMPUTA_UPDATE_INVALID_COMMAND" };
  }
  ...
  if (authorId !== resolveDiscordComputaOwnerUserId()) {
    ...
    return { ok: false as const, code: "DISCORD_COMPUTA_UPDATE_FORBIDDEN" };
  }
  ...
  const createResult = await createDiscordChannelMessage({
    ...
    body: buildDiscordComputaFormattedUpdatePayload(command),
  });

  if (!createResult.ok) {
    console.error("[discord-message-command] computa post update failed", {
      requestId: randomUUID(),
  ...
  return { ok: true as const, action: "posted" };
}
  ...
  try {
    ...
    const dispatchers = [
      () => dispatchFeedbackInteraction({
        ...
        buildDiscordFeedbackManageLookupModalResponse,
        buildDiscordEphemeralMessageResponse,
        handleBugStatusInteraction,
        handleFeedbackCompletionReviewInteraction,
        handleFeedbackWithdrawInteraction,
        handleSetupFeedbackInteraction,
        handleFeedbackSubmitPickerSelection,
        handleFeedbackSubmitCreateButton,
        buildFeedbackUpdatePickerOpenResponse,
        handleFeedbackUpdatePickerButton,
        handleFeedbackUpdatePickerSelection,
        handleFeedbackManageEditButton,
        handleFeedbackManageWithdrawButton,
        handleDeferredFeedbackCreateModalSubmit,
        handleFeedbackManageLookupModalSubmit,
        handleFeedbackUpdateModalSubmit,
        handleFeedbackWithdrawModalSubmit,
        handleFeedbackWithdrawSelectedModalSubmit,
      }),
      () => dispatchSpotifyInteraction({
        interaction,
        jsonResponse,
        handleSetupSpotifyClubInteraction,
        handleSpotifyInteraction,
        handleJamLobbyInteraction,
        handleJamQueueInteraction,
        handleSpotifyClubButtonInteraction,
        handleSpotifyQueueSuggestModalSubmit,
        handleSpotifyQueueSearchModalSubmit,
      }),
      () => dispatchVerificationInteraction({
        interaction,
        jsonResponse,
        buildDiscordVerifyModalResponse,
        handleSetupVerifyInteraction,
        handleVerifyCleanupInteraction,
        handleVerifyLockdownInteraction,
      ...
      () => dispatchModerationInteraction({
        ...
        handleWarningClearInteraction,
        handlePurgatorySetupInteraction,
        handlePurgatoryInteraction,
      }),
      ...
      () => dispatchUpdatesInteraction({
        interaction,
        jsonResponse,
        handleUpdateLatestInteraction,
        handleUpdatePublishInteraction,
        handleUpdateSkipInteraction,
        handleUpdatePublishModalSubmit,
      }),
    ];
