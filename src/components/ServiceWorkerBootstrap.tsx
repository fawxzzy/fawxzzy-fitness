...
const DEV_CACHE_RESET_KEY = "fawxzzy:fitness:dev-cache-reset-build";
...
export function ServiceWorkerBootstrap() {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV !== "production") {
      void (async () => {
        const targetBuildId = CURRENT_APP_BUILD_ID;
        const resetAppliedForBuild = window.sessionStorage.getItem(DEV_CACHE_RESET_KEY) === targetBuildId;
        ...
        if (!resetAppliedForBuild) {
          try {
            window.sessionStorage.setItem(DEV_CACHE_RESET_KEY, targetBuildId);
          } catch {
            // Ignore storage failures and keep the app usable.
          }
