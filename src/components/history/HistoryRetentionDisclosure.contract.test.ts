...
test("History no longer owns Achievements presentation", () => {
  assert.doesNotMatch(historyClientSource, /HistoryAchievementsSurface/);
  assert.doesNotMatch(historyClientSource, /buildHistoryAchievements/);
});
