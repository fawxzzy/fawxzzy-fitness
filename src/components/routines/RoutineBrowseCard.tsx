...
const ROUTINES_BROWSE_CARD_REST_PREVIEW_TILE_CLASS_NAME = "min-h-[3.7rem] content-center gap-0.5 border-[rgb(var(--accent-yellow-on)/0.5)] bg-[rgb(var(--accent-yellow-off)/0.1)] py-1.5";
...
const ROUTINES_BROWSE_CARD_PREVIEW_TAG_ROW_CLASS_NAME = "flex w-full min-w-0 flex-wrap items-center justify-center gap-1.5";
...
const ROUTINES_BROWSE_CARD_SUMMARY_ROW_CLASS_NAME = "flex flex-wrap items-center justify-center gap-1.5 px-1";
...
function renderRoutinePreviewHeader(day: NonNullable<RoutineBrowseCardItem["previewDays"]>[number]) {
  const statusIcon = day.isCompleted
    ? renderRoutineTag("DONE")
    : day.isSkipped
      ? renderRoutineTag("SKIPPED")
      : null;

  return (
    <div className={ROUTINES_BROWSE_CARD_PREVIEW_HEADER_CLASS_NAME}>
      <span className={cn(
      ...
      <div className={ROUTINES_BROWSE_CARD_PREVIEW_META_LINE_CLASS_NAME}>
        ...
        {statusIcon ? <span className="self-center">{statusIcon}</span> : null}
      </div>
      <MetricAccentBar variant="thin" className="mt-1 w-16 opacity-85" />
    </div>
  );
}
...
function renderRoutinePreviewMeta(day: NonNullable<RoutineBrowseCardItem["previewDays"]>[number]) {
  ...
  if (day.splitSummary) {
    ...
    if (splitParts.length > 0) {
      return (
        <div className={ROUTINES_BROWSE_CARD_PREVIEW_TAG_ROW_CLASS_NAME}>
          {splitParts.map((part) => (
...
function renderRoutineSummaryTags(routine: RoutineBrowseCardItem) {
  ...
  return (
    <div className={ROUTINES_BROWSE_CARD_SUMMARY_ROW_CLASS_NAME}>
      {parts.map((part) => (
