...
test("re-importing the same snapshot with merge stays idempotent", async () => {
  const admin = createAdminStub({
    exercises: [{
      id: "exercise-1",
      name: "Bench Press",
  ...
  assert.equal(admin.tables.sessions.length, 1);
  assert.equal(admin.tables.session_exercises.length, 1);
  assert.equal(admin.tables.sets.length, 1);
  assert.deepEqual(admin.tables.sets[0], {
    id: "set-1",
    client_log_id: "stable-set-1",
...
test("skips orphaned child rows instead of crashing the import", async () => {
  const admin = createAdminStub({
    exercises: [{
      id: "exercise-1",
      name: "Bench Press",
  ...
  const snapshot = createSnapshot({
    ...
    sessions: [{
      legacy_session_id: "session-1",
      legacy_routine_id: "missing-routine",
      performed_at: "2026-04-17T00:00:00.000Z",
      routine_day_index: null,
      name: "Workout",
      routine_day_name: null,
    ...
    session_exercises: [
      {
        ...
        legacy_session_id: "session-1",
        legacy_exercise_id: "exercise-1",
        legacy_routine_day_exercise_id: "missing-routine-day-exercise",
        position: 0,
        performed_index: 0,
        is_skipped: false,
        notes: null,
      ...
      {
        legacy_session_exercise_id: "session-exercise-2",
        legacy_session_id: "missing-session",
        legacy_exercise_id: "missing-exercise",
        legacy_routine_day_exercise_id: null,
        position: 1,
        performed_index: 1,
        is_skipped: false,
    ...
    sets: [
      ...
      {
        legacy_set_id: "set-2",
        legacy_session_exercise_id: "missing-session-exercise",
        client_log_id: null,
        set_index: 1,
        weight: 135,
        weight_unit: "lbs",
  ...
  assert.equal(admin.tables.sessions.length, 1);
  assert.equal(admin.tables.sessions[0].routine_id, null);
  assert.deepEqual(
    admin.tables.session_exercises.map((row) => ({
      id: row.id,
  ...
  assert.deepEqual(
    admin.tables.sets.map((row) => row.id),
    ["set-1"],
  );
  assert.equal(summary.importedCounts.sessions, 1);
  assert.equal(summary.importedCounts.session_exercises, 1);
  assert.equal(summary.importedCounts.sets, 1);
  assert.deepEqual(summary.affectedExerciseIds, ["exercise-1"]);
});
