...
test("builds clean section-based CSV tables without the old wide workout log", () => {
  const payload = buildPayload();
  const tables = buildAccountWorkoutExportCsvTables(payload);
  const csv = serializeAccountWorkoutExportCsv(tables);
  ...
  assert.deepEqual(
    ...
    [
      ...
      "history_exercises",
      "history_sets",
      "history_progression_events",
      "routines",
      "routine_days",
      "routine_exercises",
    ],
  );
  assert.match(csv, /table,history_sessions/);
  assert.match(csv, /table,history_progression_events/);
  assert.doesNotMatch(csv, /table,workout_log/);
  assert.doesNotMatch(csv, /session_notes/);
});

test("builds clean export sections and omits blank fields from history set rows", () => {
  const payload = buildPayload();
  const sections = buildAccountWorkoutExportSections(payload);
  const historySetRow = sections.find((section) => section.key === "historySets")?.rows[0];

  assert.ok(historySetRow);
  assert.equal(historySetRow?.setId, "set-1");
  assert.equal(historySetRow?.effort, 8);
  assert.equal("timeSeconds" in (historySetRow ?? {}), false);
  assert.equal("dist" in (historySetRow ?? {}), false);
});

test("builds a json export document with section metadata", () => {
  const payload = buildPayload();
  const document = buildAccountWorkoutExportJsonDocument(payload);

  assert.equal(document.metadata.scopeLabel, "All");
  assert.deepEqual(
    document.metadata.includedSections.map((section) => section.key),
    [
      "historySessions",
      "historyExercises",
      "historySets",
  ...
  assert.equal(document.data.historySessions?.[0]?.sessionId, "session-1");
});
...
test("filters export payload to history-only data", async () => {
  const rows = {
    routines: [{
      id: "routine-1",
      user_id: "user-1",
  ...
  assert.equal(payload.routines.length, 0);
  assert.equal(payload.metadata.counts.completedSessions, 1);
});

test("filters export payload to routines-only data", async () => {
  const rows = {
    routines: [
      {
        id: "routine-1",
  ...
  assert.deepEqual(payload.routines.map((routine) => routine.id), ["routine-1"]);
  assert.deepEqual(payload.routineExercises.map((exercise) => exercise.id), ["routine-exercise-1"]);
  assert.equal(payload.sessions.length, 0);
  assert.equal(payload.progressionEvents.length, 0);
});
