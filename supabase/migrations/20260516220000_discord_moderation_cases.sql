create table if not exists public.discord_moderation_cases (
  ...
  constraint discord_moderation_cases_action_check check (action in ('purgatory')),
  constraint discord_moderation_cases_status_check check (status in ('active', 'released', 'expired', 'failed')),
