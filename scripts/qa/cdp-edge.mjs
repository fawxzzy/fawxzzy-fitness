...
function buildActionResultDescriptor(action, index) {
  const defaultLabel = action.type === "waitForText"
    ? `Wait for text: ${String(action.text ?? "")}`
    : action.type === "assertExpression"
      ? String(action.message ?? "Render assertion")
      : action.type;

  return {
    id: String(action.id ?? `${action.type}-${index + 1}`),
    label: String(action.label ?? defaultLabel).trim() || defaultLabel,
  };
}

class CaptureActionError extends Error {
...
async function captureOnce(config) {
  ...
  try {
    ...
    for (let index = 0; index < actions.length; index += 1) {
      ...
      const descriptor = buildActionResultDescriptor(action, index);
      ...
      try {
        ...
        for (let skippedIndex = index + 1; skippedIndex < actions.length; skippedIndex += 1) {
          ...
          const skippedDescriptor = buildActionResultDescriptor(skippedAction, skippedIndex);
          actionResults.push({
            ...skippedDescriptor,
            status: "skipped",
            reason: `Skipped after ${descriptor.id} failed.`,
          });
        }

        throw new CaptureActionError(reason, {
          url: String(config.url),
          screenshotPath: path.resolve(String(config.outPath)),
          actionResults,
        });
      }
    }
...
async function main() {
  ...
  try {
    const summary = await captureOnce(config);
    ...
    if (error instanceof CaptureActionError && error.summary) {
