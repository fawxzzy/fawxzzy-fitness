...
function mergeRefreshedSession(artifact, nextSession) {
  ...
  return {
    ...artifact,
    version: typeof artifact.version === "number" ? artifact.version : 2,
    createdAt: artifact.createdAt ?? artifact.generatedAt ?? refreshedAt,
    generatedAt: refreshedAt,
    expiresAt: Number.isFinite(expiresAtEpochSeconds) ? new Date(expiresAtEpochSeconds * 1000).toISOString() : null,
    expiresAtEpochSeconds: Number.isFinite(expiresAtEpochSeconds) ? expiresAtEpochSeconds : null,
    authStateSummary: {
      ...(artifact.authStateSummary ?? {}),
      status: "authenticated",
      summary: "valid QA session",
