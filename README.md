...
Keep the mirror env file outside the repo root under `<ATLAS_SECRETS>/local/...`. The current local target for this repo is Postgres on `127.0.0.1:5432`, so `LOCAL_DATABASE_URL` must point there rather than the Supabase CLI port `54322`.

```powershell
cd repos/fawxzzy-fitness
node .\scripts\sync-prod-to-local.mjs --env <ATLAS_SECRETS>\local\fitness-prod-to-local.env --yes
```
