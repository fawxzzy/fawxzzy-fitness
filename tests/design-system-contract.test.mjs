...
test("live app primitives bridge the frozen design-system pack", () => {
  ...
  assert.ok(exerciseGoalForm.includes("appTokens.measurementInlineValueLabelLower"));
  assert.ok(sessionTimers.includes("appTokens.currentSessionLoggerSummaryCard"));
  assert.ok(sessionTimers.includes("appTokens.currentSessionDeleteActionRail"));
