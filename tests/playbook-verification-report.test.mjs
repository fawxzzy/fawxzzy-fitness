...
const reportPath = path.join(repoRoot, "exports", "fitness.playbook.verification.report.v1.json");
const schemaPath = path.join(atlasRoot, "schemas", "atlas.playbook.verification.report.v1.json");
const adoptionEvidencePath = path.join(repoRoot, "exports", "fitness.playbook.adoption.evidence.v1.json");
const commandsDocPath = path.join(repoRoot, "docs", "COMMANDS.md");
const verificationDocPath = path.join(repoRoot, "docs", "ops", "FITNESS-PLAYBOOK-VERIFICATION.md");
...
test("fitness verification report validates against the ATLAS root schema", () => {
...
test("fitness verification report keeps the targeted verified claim explicit", () => {
  const report = loadJson(reportPath);
  const adoptionEvidence = loadJson(adoptionEvidencePath);

  assert.equal(report.repo.repo_id, "fitness");
...
test("fitness verification report references the live repo command surface", () => {
  const report = loadJson(reportPath);
