...
test("fitness ATLAS contract lane stays wired to docs, scripts, and workflow", () => {
  ...
  assert.ok(adoptionDoc.includes(`ATLAS reusable workflow contract pinned at commit \`${ATLAS_CONTRACTS_REF}\``));
  assert.ok(workflow.includes("name: atlas-contracts"));
  assert.ok(workflow.includes("name: Contract Check"));
  assert.ok(workflow.includes("run: npm run test:atlas-contracts"));
  assert.ok(workflow.includes("name: Verify"));
  assert.ok(workflow.includes("run: npm run verify"));
  assert.ok(workflow.includes(`source_contract_commit: ${ATLAS_CONTRACTS_REF}`));
});
