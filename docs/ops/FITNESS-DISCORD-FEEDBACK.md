...
## Submit flow
The public submit flow is picker-first.

User-facing flow:
- `Submit` opens a dismissible ephemeral picker
- the picker lets the user switch between `Bug` and `Feature`
- the picker then shows a type-specific `Create Bug` or `Create Feature` button
- the create button opens the matching modal
- the top modal field no longer asks for `Bug` or `Feature`
...
- Once a resolved card has a `completed` board copy, duplicate source-board copies should be deleted with `npm run discord:feedback:archive-completed-duplicates -- --apply`.
...
- Acceptance Criteria are generated deterministically only when the submit or edit flow did not receive explicit section overrides
...
- when a feature card has explicit acceptance-criteria lines stored in the bounded row, the forum starter post and exports should use those stored criteria instead of the generic fallback list
- when a feature card has an explicit `User Story` override, the forum starter post should use that stored user story instead of the generic fallback
- this keeps Discord cards, board exports, and reviewed task packets aligned
- the forum starter post should spend the remaining body budget on the stored feature description after preserving metadata, acceptance criteria, and evidence
- do not replace a scoped approved feature description with a shorter generic paraphrase unless Discord body limits force truncation

Section override rule:
- Bug cards can carry explicit overrides for `Expected behavior`, `Actual behavior`, `Steps to reproduce`, and `Acceptance Criteria`
...
- the first estimate is deterministic from the bounded feedback row
- staff may refine estimates later as implementation history improves the calibration
- `Closed`
- `Duplicate`
- `Spam`
- `Withdrawn`
...
- allow the original reporter or staff
- resolve by full UUID, short id, thread id, or thread URL
- remain a fallback path; the main user flow reaches withdraw from `Edit`
