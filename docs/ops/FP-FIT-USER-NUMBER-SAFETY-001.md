...
1. Takes an `ACCESS EXCLUSIVE` lock on `public.profiles` first, draining profile writes before any allocator lock is requested.
2. Resolves the exact owner and `xmin` of `public.real_user_number_seq`, requires the owner to equal `CURRENT_USER`, and executes the supported no-op `ALTER SEQUENCE ... OWNER TO CURRENT_USER` operation to acquire the sequence heavyweight lock. It re-reads owner and `xmin` and fails unless both are unchanged. The profile and sequence locks are retained through allocator validation, privilege cutover, and transaction commit; no sequence configuration or state is changed.
3. Fails closed unless the assignment trigger and function are exact and enabled; the named sequence resolves to exactly one authoritative catalog row, increments by one, cannot cycle, uses `CACHE 1`, and has an effective-next value above every non-null member number already reserved by any profile; and `profiles_user_number_uq` is the live, ready, valid one-key `user_number` partial unique index on `public.profiles`. `CACHE 1` is required because a larger cache can leave lower preallocated values in another session even when `last_value` appears above the observed high-water mark. Compaction objects must form either the exact legacy set or the fully retired set. For a non-empty profile denominator, every human has a non-null, non-negative, unique number and assignment metadata, exactly one existing profile reserves `#0` as a human with assignment metadata, and every numbered profile is human. A pristine zero-profile migration chain is admitted only when every profile/member-number denominator is empty and the integer sequence retains the exact historical state created by migrations `044` and `056`: start/minimum/current/effective-next `1`, maximum `2147483647`, `is_called=false`, increment `1`, `CACHE 1`, and no cycling. Any empty-but-advanced, empty-but-reconfigured, non-empty-without-`#0`, unnumbered-human, or otherwise partial state fails before object mutation.
4. Transactionally reinstalls the immutable historical `public.is_automation_auth_user(uuid)` SQL/STABLE/SECURITY DEFINER definition, `search_path`, postgres ownership, and client execution revokes before the assignment function can use it.
5. Drops the compaction trigger, delete wrapper, and compactor with `IF EXISTS` and `RESTRICT`, never `CASCADE`.
6. Redefines insert assignment so automation profiles are always unnumbered and every new human receives `nextval`, regardless of caller-supplied identity values.
7. Adds `public.enforce_immutable_profile_member_identity()` and `profiles_enforce_immutable_member_identity_before_update`; same-value updates pass, while changes to `user_number`, `user_kind`, or `user_number_assigned_at` fail.
8. Removes sequence mutation/allocation privileges from public client roles and `service_role`; `service_role` retains read-only `SELECT`, while the postgres-owned `SECURITY DEFINER` assignment function remains the allocator.
9. Adds comments that freeze immutable, never-reused semantics.
...
- Every human profile missing `user_number` remains a failure; installing immutability must never freeze an unnumbered human.
