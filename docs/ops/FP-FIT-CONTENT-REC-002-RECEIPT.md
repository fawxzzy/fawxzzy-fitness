...
The verifier resolves the requested commitish exactly once to an immutable 40-hex commit SHA, fails closed when resolution is unavailable or ambiguous, and uses and reports only that SHA for every manifest, tree, blob, ancestry, and allowlist read. Every verifier-owned Git subprocess overrides inherited state with `GIT_NO_REPLACE_OBJECTS=1`, so repository-local replacement refs cannot substitute another commit or blob behind the reported SHA. Its public CLI accepts only an optional `--ref <commitish>` and rejects every unsupported argument shape. Mutable symbolic refs, replacement refs, worktree content, and index content are not acceptance inputs.
...
- focused tests: PASS, 26/26, including forged replacement-object rejection across resolution and manifest/tree/blob/ancestry/allowlist reads, immutable symbolic-ref pinning, explicit full-SHA and default-HEAD behavior, unavailable/ambiguous fail-closed resolution, exact-ref CLI behavior, deterministic CLI argument rejection, committed manifest-drift rejection, worktree/index isolation, and path/byte-contract drift rejection;
