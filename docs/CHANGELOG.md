...
### WHAT
- Rebound the active Playbook runtime fallback from the retired `ZachariahRedfield` GitHub release to the source-bound `fawxzzy/playbook` v0.54.0 fallback asset, and added an exact default-acquisition regression.
...
### WHY
- Clean-environment verification must remain reproducible after the legacy GitHub account retirement without relying on deleted repository coordinates or unverifiable package provenance.
...
- Documented the local Pillow dependency for board generation in the mobile regression QA docs.
