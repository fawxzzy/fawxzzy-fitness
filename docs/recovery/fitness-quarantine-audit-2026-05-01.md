# Fitness Quarantine Audit

Canonical repo: repos/fawxzzy-fitness
Quarantine folder: <ATLAS_ROOT>/repos/fawxzzy-fitness-quarantine

## Counts by target/status
- src/components /  different: 67
- src/app /  different: 39
- src/lib /  different: 36
- scripts /  only_quarantine: 20
- scripts /  different: 11
- supabase /  only_quarantine: 7
- src/features /  different: 5
- public/exercises /  different: 3
- tests /  different: 2
- src/generated /  different: 2
- next.config.mjs /  only_canonical: 1
- package.json /  different: 1
- middleware.ts /  only_canonical: 1
- vercel.json /  different: 1
- public/app /  different: 1
- supabase /  different: 1
- docs/PLAYBOOK_NOTES.md /  only_canonical: 1
- docs/LOCAL-PROD-DATA-SYNC.md /  different: 1

## Notes
- Quarantine is not a standalone app repo; its Git top-level resolves to `<ATLAS_ROOT>` and remote is `ATLAS.git`.
- This inventory is filesystem content comparison only.
- No files were modified by this audit.
