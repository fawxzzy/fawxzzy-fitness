...
- WHAT changed: The Fitness session-handoff endpoint now returns a fixed readiness contract only when the runtime has the exact master Supabase audience, a structurally valid master-bound anonymous validator key matching a separately sealed server-only SHA-256 fingerprint, an immutable deployment source commit, and an available durable one-time handoff store. Missing, malformed, legacy, conflicting, or unavailable runtime identity fails closed before a handoff record is written. The provider-free loopback fixture supplies the same contract with a deterministic synthetic source identity.
- WHY it changed: A portal producer must not infer that Fitness is ready for cross-origin session handoff from an HTTP success alone. Explicit runtime attestation prevents activation against a legacy Auth audience, an anonymous validator bound to another project, an unidentified deployment, or an unavailable single-use store while preserving non-echoing failure behavior.
- Rule: Cross-origin session handoff activation requires exact consumer runtime attestation, including the credential used to validate the submitted session; local fixtures may inject deterministic readiness but production must derive it from immutable runtime evidence.
- Failure Mode: Issuing a handoff without proving the destination Auth audience, validator credential, deployed source, and durable store can create login loops, cross-audience token rejection, unhandled configuration errors, or replay-prone session transport.
...
## 2026-09-12 - Keep Fitness account controls aligned to one viewport contract

- Type: Account UI parity + Accessibility + Mobile safe-area behavior
- WHAT changed: The shared Fitness auth shell now keeps the product/title group safe-area-aware at the top, centers each complete field group in the usable viewport, reserves fixed secondary and legal rows above the bottom action, and places each divider on the screen centerline. Login, signup, recovery request, recovery callback, and new-password forms use one auth-field wrapper with centered editable text, native caret/selection/autofill behavior, one fieldset focus indicator, invalid-state precedence, forced-colors support, and one product-owned password reveal control.
- WHY it changed: Independently positioned screen variants had drifted in field alignment, legal placement, focus treatment, and bottom inset handling. The shell also hid the inherited caret and allowed browser-native password reveal chrome to duplicate the product eye.
- Rule: Account-screen anatomy belongs to the shared auth shell and auth-field contract; product identity and green presentation remain Fitness-owned. Center the field group independently of title/footer height on normal viewports, fall back to document flow on short viewports, and never trade native editing or password-manager semantics for visual centering.
...
- Rule: Same-origin Account navigation must refresh the local session before rendering, and an Account-only authentication failure must preserve its same-origin destination as encoded login state; it must never promote a nested caller value into an external redirect origin.
