...
## 2026-08-04 - Wire session-start to the atomic RPC and remove the now-superseded rollback helper
- Type: Activation (source-only; kept provider-gated behind a merge trigger, not applied to any live database)
- WHAT changed: `createSessionFromDay` in `src/lib/start-session.ts` no longer performs a source-level existence check, a direct `sessions` insert, a direct `session_exercises` insert, or a manual rollback-on-failure -- it now calls `createSessionAtomicallyFromDay` (new file `src/lib/session-start-activation.ts`), which builds the exercise-intent payload (relocated verbatim from `start-session.ts`, unchanged mapping logic) and calls `startSessionFromDayAtomicV1` (the RPC adapter merged in the prior wave). `session-start-integrity.ts` and its test file are deleted -- `findExistingInProgressSession`/`rollbackFailedSessionStart` had zero remaining callers once this activation landed, so they were removed rather than left as dead code. Added `src/lib/session-start-activation.test.ts` (13 tests covering: every current goal column mapped correctly, deterministic exercise order, the exact RPC-argument key set, successful activation, exact routine/day identifiers reaching the RPC, empty exercise lists, duplicate-request idempotency, provider-error propagation, a simulated missing-RPC PostgREST error, thrown-error handling, malformed-response rejection, and two static-source-contract checks proving no direct table insert or manual rollback remains reachable). Updated `src/lib/session-start-atomicity.test.ts`'s "not yet wired in" test, whose premise this activation makes obsolete, to instead prove the full call chain reaches the RPC. Updated `src/lib/dal/planner-routine-create.test.ts`'s cross-contract test (which reads `start-session.ts` as raw text to guard against goal-column drift against the planner RPC) to read the relocated mapping logic from its new location in `session-start-activation.ts`. Wired a new `test:session-start-activation` npm script; removed the now-dead `test:session-start-integrity` script.
- WHY it changed: the migration and RPC were merged source-only in the prior wave, deliberately not wired in because switching the call site before the migration was applied to the live database would have broken session creation for every user in production. That risk no longer applies to *this* PR's own source (it's safe to merge at any time -- the RPC-calling code simply won't work until the migration is applied), but the resulting code path only functions correctly once the migration is live. This PR is therefore explicitly held provider-gated: `SOURCE_READY__WAITING_PROVIDER_APPLICATION`. See `FITNESS_ATOMICITY_PROVIDER_PACKET_V2.md` for the exact pre-apply check, apply command, and required readback before this PR may be marked ready or merged.
- Rule: when activating a previously source-only, deliberately-unwired change, don't silently keep a stale "not yet wired in" test passing by accident (it will pass vacuously once the wiring moves to a different file than the test originally watched) -- audit every test that encoded the *old* invariant and either update it to assert the *new* invariant or delete it; a test whose name and assertion have drifted apart is worse than no test.
- Failure Mode: merging and deploying this PR's code before the migration is applied to the live database would make every `startSessionForActiveRoutineDay`/`startSessionForRoutineDay` call fail (the RPC would not exist yet) -- there is deliberately no fallback to the old direct-insert path, since a silent fallback would quietly reintroduce the concurrent-duplicate-session race the RPC exists to close. This is why the PR carries an explicit merge gate rather than a runtime feature flag: a runtime fallback would hide the exact failure mode this migration was built to prevent.
- Decision: `ACTIVATION_ADAPTER_READY`. Scope: `src/lib/start-session.ts`, new `src/lib/session-start-activation.ts` + test, deletion of `src/lib/session-start-integrity.ts` + test, `package.json` (script rename only), one comment fix in `src/lib/session-start-atomicity.ts` (its "not yet called" header comment was now false), and the two adjacent test files updated to track the relocated code. No production behavior changes take effect until the migration is separately applied and this PR is separately merged.
- Evidence: `src/lib/start-session.ts`, `src/lib/session-start-activation.ts`, `src/lib/session-start-activation.test.ts`, `src/lib/session-start-atomicity.ts`, `src/lib/session-start-atomicity.test.ts`, `src/lib/dal/planner-routine-create.test.ts`; `FITNESS_ATOMICITY_PROVIDER_PACKET_V2.md`.
- Status: Applied (source only -- migration not applied to any live database; this PR must not be marked ready/merged before provider readback per the packet)
