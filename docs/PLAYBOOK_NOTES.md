...
- Type: Pattern
- WHAT changed: The history/logged-session detail route now wraps the lower exercise-card lane in the same bounded glass scroll-box treatment used by progression measurement panels, sizes that shell from the natural card stack in collapsed mode, keeps the shell filled to the footer-safe viewport in expanded mode, and gives the expanded exercise disclosure stack a real full-height flex parent so the exercise info panel can actually fill the box while the notes row stays anchored at the bottom.
- WHY it changed: Logged session cards needed the same footer-safe, bounded scrolling behavior already proven on progression measurement input screens, but the shell also had to stop collapsing down to the currently expanded exercise content and the expanded disclosure stack needed a true inherited height, otherwise the exercise info area could collapse to a thin strip even though the outer box was still present.
