...
- Type: Guardrail
- Summary: When quarantine recovery adds exercise catalog data, seed migrations, or schema-alignment SQL without changing the normalized runtime trees, the branch must record whether the current app code actually depends on those migrations before promotion.
