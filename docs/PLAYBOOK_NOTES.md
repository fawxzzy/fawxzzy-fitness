...
## 2026-02-26 — Prefer slug-keyed text SVG maps for large icon catalogs
- Type: Pattern
- Summary: For large exercise/icon catalogs, use deterministic slug keys + typed icon specs and render inline SVG variants instead of committing binary media files.
- Suggested Playbook File: patterns/server-client-boundaries.md
- Rationale: Text-only assets simplify review flow, avoid binary restrictions, and preserve consistent UI rendering without storage coupling.
- Evidence: src/lib/exercises/slug.ts, src/lib/exercises/iconSpec.ts, src/lib/exercises/exerciseIconMap.ts, src/components/exercises/ExerciseIcon.tsx, src/components/ExercisePicker.tsx
- Status: Proposed
