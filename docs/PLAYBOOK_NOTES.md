...
## 2026-07-31 - Present rest days as deliberate cards on Today and reuse the existing Routine Overview rest-day treatment

- Type: Bug fix
- WHAT changed: Added a shared, non-interactive `RestDayCard` primitive and wired it into the Today screen (in-progress-session exercise list, closed day-picker view) so a rest day renders a deliberate card instead of nothing or plain text. Extracted `REST_DAY_CARD_COPY` to a dependency-free module as single source of truth. Also updated `EditRoutineDaysSection.tsx` and its resolver for consistency with the same rest-day rules.
...
- Caveat: `EditRoutineDaysSection.tsx` is confirmed unreachable from any current route (`/routines/[id]/edit` renders only `EditRoutineAutosaveForm`; the live, reorderable day list is `RoutineOverviewDayCard` via `/routines/[id]`, which already had the pre-existing rest-day accent). The changes to `EditRoutineDaysSection.tsx` and its resolver/tests are correct and consistent but have no live user-facing effect until/unless that component is wired into a route — flagged as a separate follow-up decision, not resolved by this change.
- Evidence: `src/components/day-list/RoutineDayCardPresentation.tsx`, `src/features/day-state/restDayCardCopy.ts`, `src/app/today/TodayExerciseRows.tsx`, `src/app/today/TodayDayPicker.tsx`, `src/app/today/todayRestDayCard.ts`, `src/app/today/page.tsx`, `src/app/today/TodayClientShell.tsx`, `src/app/routines/[id]/edit/EditRoutineDaysSection.tsx`, `src/app/routines/[id]/edit/editRoutineDayRowPresentation.ts`, `docs/today-state-matrix.md`
- Status: Applied
