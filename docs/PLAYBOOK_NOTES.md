...
- WHAT changed: `BottomSheet` now keeps Tab and Shift+Tab navigation inside its dialog, restores focus to the opener after Escape or scrim close, and retains the existing scroll-lock, portal, and reduced-motion behavior. The Playbook official fallback downloader now makes at most one additional attempt when a recognized transient socket closure occurs either before a response is received or while its body is read.
- WHY it changed: A keyboard user could tab out of an open BottomSheet dialog. Separately, the exact Playbook verification artifact download failed twice on the same external `SocketError: other side closed` transport class even though the official artifact endpoint subsequently recovered; retrying only that bounded transient class lets normal CI distinguish temporary transport loss from deterministic download or validation failure.
- Rule: Modal dialog focus must remain contained until the dialog closes. Official fallback acquisition may retry exactly once only for a recognized transient transport failure during fetch or body acquisition; HTTP failures, write failures, and empty-artifact validation failures remain terminal and visible.
- Failure Mode: Letting Tab escape a modal strands keyboard focus behind an active dialog. Treating every download failure as retryable can mask deterministic HTTP or artifact-integrity failure, while treating a one-off socket close as terminal makes a healthy external artifact endpoint needlessly flaky.
- Decision: The retry budget is two total attempts and is deliberately limited to transport exceptions. The official URL, final URL reporting, non-OK HTTP handling, on-disk write, and nonempty-artifact validation semantics are unchanged. No provider, deployment, runtime execution, or production behavior changed.
